...
const Product = ({ title, categories, images, price, minPrice, maxPrice, text }) => {
    return <section className='responsive-container w-full mt-16 lg:mt-[112px] lg:max-h-[calc(100svh-112px)] py-16 lg:pb-16 lg:pt-12 flex flex-col lg:flex-row gap-10 lg:gap-20'>
        <div className='w-full lg:w-[60%] shrink-0 h-full'>
            {images?.length > 1 && <Slider className="aspect-[16/12] lg:aspect-[16/12] xl:aspect-[16/10] lg:wh-full !rounded-3xl" spaceBetween={16}>
            ...
            {images?.length == 1 && <Image src={images[0].url ?? "/placeholder.svg"} alt={images[0].alt ?? title} width={0} height={0} sizes='100vw' className='rounded-3xl w-full h-full object-cover' />}
        </div>
        <div className='flex md:items-start flex-col gap-6 lg:gap-8 w-auto lg:overflow-auto max-h-full'>
            <div className='flex flex-col'>
                ...
                <Heading level="2" tag="h1" className="mt-3">{title}</Heading>
                <Heading level="3" className="mt-5">
                    {(minPrice && !maxPrice) && <>A partir de <strong className='text-red-800'>{new Intl.NumberFormat("fr-FR").format(minPrice)}</strong> €</>}
                    {(minPrice && maxPrice) && <>De <strong className='text-red-800'>{new Intl.NumberFormat("fr-FR").format(minPrice)}</strong> à <strong className='text-red-800'>{new Intl.NumberFormat("fr-FR").format(maxPrice)}</strong> €</>}
                </Heading>
            </div>
            {text && <p>{text}</p>}
