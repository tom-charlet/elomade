...
const Product = ({ title, categories, images, price, minPrice, maxPrice, text, available }) => {
    ...
    return <section className='responsive-container w-full mt-16 lg:mt-[112px] lg:min-h-[calc(100svh-112px)] py-16 lg:pb-16 lg:pt-12 flex flex-col lg:flex-row gap-10 lg:gap-20'>
        {load ? <>
            <motion.div variants={isMobile ? fadeUp : fadeRight} initial="hidden" whileInView="visible" viewport={{ once: true }} className='w-full lg:w-[60%] shrink-0 h-full'>
                {images?.length > 1 && <Slider
                    className="!aspect-[11/14] md:!aspect-[16/12] xl:!aspect-[16/10] lg:!w-full !rounded-3xl"
                    spaceBetween={16}
                    pagination
                    freeMode={true}
                    mousewheel={true}
                >
                    {images.map((item, index) => <span key={index} className='w-auto h-auto bg-beige-300'>
                        <Image key={index} src={item.url ?? "/placeholder.svg"} alt={item.alt ?? title} width={0} height={0} sizes='100vw' className='rounded-3xl w-auto h-full object-cover' />
                    </span>)}
                </Slider>}
                {(!images || images?.length == 0 || images?.length == 1) && <Image src={images?.length > 0 ? images[0]?.url : "/placeholder.svg"} alt={images?.length > 0 ? (images[0]?.alt ?? title) : "Aucune photo"} width={0} height={0} sizes='100vw' className={`rounded-3xl w-full ${!images || images?.length == 0 ? "aspect-[9/10]" : "h-auto"} h-auto md:aspect-[16/12] xl:aspect-[16/10] object-cover`} />}
            </motion.div>
        </> : <></>}
        <motion.div variants={animation("stagger")} initial="hidden" whileInView="visible" viewport={{ once: true }} className='flex md:items-start flex-col gap-6 lg:gap-8 w-auto'>
            ...
            {text && <motion.div variants={animation("fadeUp")} className='post-text text-[14px]'>
                <Text>
                    {text}
                </Text>
            </motion.div>}
            <MotionButton variants={animation("fadeUp")} href={available ? "#me-contacter" : "#"} disabled={!available} className="mt-1 lg:mt-0">{available ? "Me contacter" : "Indisponible"}</MotionButton>
        </motion.div>
    </section>
}
