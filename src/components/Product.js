    ...
    return <section className='responsive-container w-full mt-16 lg:mt-[112px] py-16 lg:pb-16 lg:pt-12 flex flex-col lg:flex-row gap-10 lg:gap-20'>
        <motion.div variants={isMobile ? fadeUp : fadeRight} initial="hidden" whileInView="visible" viewport={{ once: true }} className='w-full lg:w-[60%] shrink-0 h-full'>
            {images?.length > 1 && <Slider
                ...
                mousewheel={true}
            >
                {images.map((item, index) => <div key={index} className='w-auto h-full relative'>
                    <Image key={index} src={item.url ?? "/placeholder.svg"} alt={item.alt ?? title} width={0} height={0} sizes='100vw' className={`bg-beige-300 overflow-hidden rounded-3xl w-auto h-full object-cover min-w-[80svw] md:min-w-[50svw] ${images?.length == 2 ? "lg:min-w-[calc(25svw-32px)]" : "lg:min-w-auto"}`} />
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className='flex md:items-start flex-col gap-6 lg:gap-8 w-auto'>
            <div className='flex flex-col'>
                ...
                <motion.div variants={fadeUp}>
                    <data className="block mt-6 text-[24px] font-semibold">
                        Prix :
                        {price && <> <strong className='text-red-700 font-semibold'>{new Intl.NumberFormat("fr-FR").format(price)} €</strong></>}
            ...
            {text && <motion.div variants={fadeUp} className='post-text'>
                <Text>
                    {text}
                </Text>
            </motion.div>}
